...
impl History {
    ...
    fn compute_next(&self) -> i64 {
        let mut nexts = vec![0 as i64; self.sequences.len()];
        for i in (0..nexts.len()-1).rev() {
            nexts[i] = self.sequences[i].last().unwrap() + nexts[i+1];
        }
        return nexts[0];
    }
}
